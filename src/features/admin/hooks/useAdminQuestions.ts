...
function normalizeDiff(v: any): "easy" | "medium" | "hard" {
  ...
  if (s === "max") return "hard" // legacy -> hard
...
export function useAdminQuestions(selectedCategory: string) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const norm = list.map((q) => ({ ...q, difficulty: normalizeDiff((q as any).difficulty) }))
  ...
  async function submitEditQuestion(e?: React.FormEvent) {
    ...
    setQuestions((prev) =>
      prev.map((x) =>
        x.id === updated.id ? { ...updated, difficulty: normalizeDiff((updated as any).difficulty) } : x,
      ),
    )
  ...
  async function exportQuestionsCSV() {
    ...
    for (const q of questions) {
      const opts = q.options.map((o) => csvEscape(o.text)).join("|") // pipe-separated
      const correctPositions = (q.correctOptionIds || [])
        .map((id) => q.options.findIndex((o) => o.id === id) + 1)
        .filter((n) => n > 0)
        .join("|")
      rows.push(
        [csvEscape(q.text), q.type, normalizeDiff((q as any).difficulty), csvEscape(opts), correctPositions].join(","),
      )
    }
  ...
  async function importQuestionsCSV(file: File) {
    ...
    const hasHeader = ["text", "type", "difficulty", "options", "correct"].every((k) => header.includes(k))
    ...
    for (let r = startRow; r < rows.length; r++) {
      ...
      const typeCell = (row[I.type] ?? "single").toString().trim().toLowerCase()
      const diffCell = (row[I.difficulty] ?? "medium").toString().trim().toLowerCase()
      const optionsCell = (row[I.options] ?? "").toString()
      const correctCell = (row[I.correct] ?? "").toString()

      const type = (typeCell === "multi" ? "multi" : "single") as QuestionDTO["type"]
      ...
      try {
        await adminQuestions.create(selectedCategory, {
          text: textCell,
          type,
          options: optTexts.map((t, idx) => ({ id: "", text: t || `Option ${idx + 1}` })),
    ...
    // refresh list
    const list = await adminQuestions.list(selectedCategory)
    setQuestions(list.map((q) => ({ ...q, difficulty: normalizeDiff((q as any).difficulty) })))
  }
