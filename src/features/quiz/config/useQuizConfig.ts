...
export function useQuizConfig() {
  ...
  const selectedCategoryFromState = location.state?.selectedCategory as string | undefined;
  ...
  const form = useForm<QuizConfigForm>({
    ...
    defaultValues: {
      ...
      numQuestions: 10,
      timeLimitMin: 15,
      difficulty: undefined, // undefined => Mixed
    },
    mode: "onChange",
  });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const filtered =
          uiDifficulty !== "mixed"
            ? list.filter((q) => toUI((q.difficulty as any) ?? "med") === uiDifficulty)
            : list;
        ...
        if (!cancelled) {
          ...
          if (count >= 0 && watched.numQuestions > count) {
            ...
            setValue("numQuestions", allowed.length ? allowed[allowed.length - 1] : 0, {
              shouldValidate: true,
            });
          }
        }
