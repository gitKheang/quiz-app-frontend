...
export function useQuizRuntime({ attemptId, toast }: UseQuizRuntimeArgs) {
  ...
  // derived
  const questions: QuestionDTO[] = useMemo(() => session?.questions ?? [], [session])
  const currentQuestion = useMemo(
    () => (questions.length ? questions[currentQuestionIndex] : undefined),
    [questions, currentQuestionIndex],
  )
  const answeredQuestionIds = useMemo(() => new Set(Object.keys(answers)), [answers])
  ...
  }, [answers, attemptId, navigate, resetQuiz, saveProgress, setSubmitting, submitQuiz, toast])
