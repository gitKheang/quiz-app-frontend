...
export default function ActionButtons({ results }: Props) {
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-4 justify-center">
      <Button onClick={() => navigate('/quiz/new')} size="lg" className="quiz-button-primary">
        <RotateCcw size={18} />
        Take Another Quiz
      </Button>

      <Button onClick={handleShareResults} variant="outline" size="lg" className="quiz-focus">
        <Share2 size={18} />
        Share Results
      </Button>

      <Button onClick={() => navigate('/leaderboard')} variant="outline" size="lg" className="quiz-focus">
        <Trophy size={18} />
        View Leaderboard
      </Button>
    </div>
