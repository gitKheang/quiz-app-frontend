...
export default function ReviewPanel({ results, questions = [] }: Props) {
  ...
  const qMap = useMemo(() => {
    const m = new Map<
      string,
      { id: string; text: string; options: Array<{ id: string; text: string }>; explanation?: string }
    >()
  ...
  const idsToTexts = (questionId: string, ids: string[]) => ids.map((id) => getOptionText(questionId, id))
    ...
    >
      ...
      <div className="space-y-3">
        {results.breakdown.map((b, idx) => {
          ...
          const userLabels =
            (b as any).userAnswerTexts?.length ? (b as any).userAnswerTexts! : idsToTexts(b.questionId, b.userAnswerIds)
          const correctLabels =
            (b as any).correctAnswerTexts?.length
              ? (b as any).correctAnswerTexts!
              : idsToTexts(b.questionId, b.correctAnswerIds)

          return (
            <Collapsible key={b.questionId} open={isOpen} onOpenChange={() => toggleOne(b.questionId)}>
              <Card className="quiz-card">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <Badge variant={b.isCorrect ? "default" : "destructive"} className="ml-2">
                ...
                <CollapsibleContent className="space-y-4 mt-4 text-sm">
                  {q?.text && <div className="font-medium text-base mb-3">{q.text}</div>}
