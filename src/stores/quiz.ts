  ...
  persist(
    (set, get) => ({
      ...
      setAnswer: (questionId, optionIds) => {
        console.log("[v0] Quiz Store - Setting answer for question:", questionId)
        console.log("[v0] Quiz Store - Option IDs:", optionIds)
        console.log("[v0] Quiz Store - Current answers before update:", get().answers)

        set((state) => {
          const newAnswers = { ...state.answers, [questionId]: optionIds }
          console.log("[v0] Quiz Store - New answers after update:", newAnswers)
          return { answers: newAnswers }
        })

        // Verify the answer was set correctly
        const verifyAnswers = get().answers
        console.log("[v0] Quiz Store - Verified stored answers:", verifyAnswers)
        console.log("[v0] Quiz Store - Answer for this question:", verifyAnswers[questionId])
      },
...
// (Optional) tiny selector helpers
export const useCurrentQuestion = () => useQuizStore((s) => s.currentQuestionIndex)
export const useQuizAnswers = () => useQuizStore((s) => s.answers)
export const useFlaggedQuestions = () => useQuizStore((s) => s.flaggedQuestions)
