...
export const questionHandlers = [
  ...
  // CREATE
  http.post("/api/categories/:categoryId/questions", async ({ params, request }) => {
    const { categoryId } = params as { categoryId: string }
    ...
    for (const [catId, arr] of Object.entries(store.questions)) {
      ...
      if (idx !== -1) {
        ...
        const updated: QuestionDTO = {
          ...
          difficulty: normalizeQuestionDifficulty((body as any).difficulty ?? (prev as any).difficulty),
