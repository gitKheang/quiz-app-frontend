 ...
 */
export function normalizeQuestionDifficulty(d: any): "easy" | "medium" | "hard" {
...
export function ensureNotIdentity<T>(original: T[], shuffled: T[]): T[] {
  const same = original.length === shuffled.length && original.every((v, i) => v === shuffled[i])
...
export function getQuestionsForCategory(
  ...
  const targetDiff = difficulty === undefined ? undefined : normalizeQuestionDifficulty(difficulty)

  const pool =
    targetDiff === undefined
      ? all // mixed
      : all.filter((q) => normalizeQuestionDifficulty((q as any).difficulty) === targetDiff)

  const usable = pool.length ? pool : all // graceful fallback if filter empty
  const selected = shuffleArray(usable).slice(0, Math.min(numQuestions, usable.length))
