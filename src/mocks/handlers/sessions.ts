...
function normalizeIncomingAnswers(payload: any): Record<string, string[]> {
  ...
  if (Array.isArray(a)) {
    ...
    return Object.fromEntries(
      a.map((it: any) => [
        ...
        Array.isArray(it?.chosenOptionIds) ? it.chosenOptionIds.map(String) : [],
      ]),
    )
  }
  if (a && typeof a === "object") {
    // { [questionId]: string[] }
    return Object.fromEntries(
      Object.entries(a).map(([k, v]) => [String(k), Array.isArray(v) ? (v as any[]).map(String) : []]),
    )
  }
  return {}
}
...
export const sessionHandlers = [
  ...
  http.post("/api/quiz-sessions", async ({ request }) => {
    ...
    const timeLimitMin = Math.max(5, Math.min((req as any).timeLimitMin ?? (req as any).timeLimitMinutes ?? 15, 60))
    ...
    const originalQuestions = getQuestionsWithAnswersForCategory(
      req.categoryId,
      requested,
      (req as any).difficulty && (req as any).difficulty !== "mixed" ? ((req as any).difficulty as any) : undefined,
    )
    ...
    const clientQuestions = originalQuestions.map((q) => {
      ...
      const seededOptions = deterministicShuffle(baseOptions, `${attemptId}:${q.id}`)
  ...
  http.get("/api/quiz-sessions/:id", async ({ params }) => {
    ...
    if (!sess) return HttpResponse.json({ error: "Session not found" }, { status: 404 })
    ...
    if (originalQuestions.length > 0 && questionOrder.length > 0) {
      ...
      const clientQuestions = orderedQuestions.map((q) => {
        const baseOptions = q.options
        const seededOptions = deterministicShuffle(baseOptions, `${id}:${q.id}`)
        const finalOptions = ensureNotIdentity(baseOptions, seededOptions)
  ...
  http.post("/api/quiz-sessions/:id/submit", async ({ params, request }) => {
    ...
    if ((!answers || Object.keys(answers).length === 0) && session?.currentAnswers) {
    ...
    if (originalQuestions.length === 0) {
      return HttpResponse.json({ error: "Scoring data not found" }, { status: 500 })
    }
    ...
    const breakdown = session.questions.map((clientQ) => {
      ...
      const mappedUserAnswers = userAnswerIds.map((selectedId) => {
        const selectedOption = clientQ.options.find((opt) => opt.id === selectedId)
        if (!selectedOption) return selectedId
        const originalOption = originalQ.options.find((opt) => opt.text === selectedOption.text)
      ...
      const userAnswerTexts = mappedUserAnswers.map((id) => originalQ.options.find((o) => o.id === id)?.text ?? id)
      const correctAnswerTexts = correctAnswerIds.map((id) => originalQ.options.find((o) => o.id === id)?.text ?? id)

      const isCorrect =
        mappedUserAnswers.length > 0 &&
        mappedUserAnswers.slice().sort().join("|") === correctAnswerIds.slice().sort().join("|")
    ...
    console.log(`[v0] Quiz submission completed for session: ${id}, score: ${score}`)
