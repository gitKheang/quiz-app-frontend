...
export function Timer({ endAt, serverNow, onTimeUp, className, size = 'md' }: TimerProps) {
  ...
  const calculateTimeRemaining = useCallback(() => {
    ...
    const remaining = Math.max(0, Math.floor((endTime - estimatedServerTime) / 1000));
    return remaining;
  }, [endAt]);
  ...
  return (
    ...
    >
      ...
      <span className={cn('tabular-nums', isCriticalTime && 'animate-quiz-shake')}>
